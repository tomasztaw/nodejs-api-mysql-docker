...
export const getPatients = async (
  ...
  console.info(
    `[${new Date().toLocaleString()}] Incoming ${req.method}${
      req.originalUrl
    } Request form ${req.rawHeaders[0]} ${req.rawHeaders[1]}`
  );
  try {
    ...
    return res
      .status(Code.OK)
      .send(
        new HttpResponse(Code.OK, Status.OK, "Patients retrived", result[0])
      );
  } catch (error: unknown) {
    console.error(error);
    return res
  ...
  try {
    ...
    if ((result[0] as Array<ResultSet>).length > 0) {
      return res
        .status(Code.OK)
        .send(
          new HttpResponse(Code.OK, Status.OK, "Patient retrived", result[0])
        );
    } else {
      return res
